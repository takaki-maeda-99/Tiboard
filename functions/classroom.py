...
def test_creds():
    ...
    if os.path.exists(f"{TOKENS_FILE_PATH}/test_token.json"):
        creds = Credentials.from_authorized_user_file(f"{TOKENS_FILE_PATH}/test_token.json", SCOPES)
    ...
    if not creds or not creds.valid:
        ...
        with open(f"{TOKENS_FILE_PATH}/test_token.json", "w") as token:
...
def main():
    ...
    user_info, course_info = async_request_user_and_course_info(headers)
    
    for course in course_info:
        print(course.get("id", ""), course.get("name", ""))
        
    
    course_ids = [course.get("id", "") for course in course_info]
    courseWork_info = async_request_courseWork_info(headers, course_ids)
    
    for courseWorks in courseWork_info:
        for courseWork in courseWorks:
            print(courseWork.get("courseId",""),courseWork.get("id", ""), courseWork.get("title", ""), convert_utc_to_jst(date_dict=courseWork.get("dueDate", {}), time_dict=courseWork.get("dueTime", {})))
    
    course_and_courseWork_ids = []
    for courseWorks in courseWork_info:
        for courseWork in courseWorks:
            course_and_courseWork_ids.append((courseWork.get("courseId", ""), courseWork.get("id", "")))
            
    submissions_info = async_request_submissions_info(headers, course_and_courseWork_ids)
    
    for submission in submissions_info:
        print(submission.get("courseWorkId", ""), submission.get("state", ""))
    
    print([len(courseWork) for courseWork in courseWork_info])
    print(len(submissions_info))
