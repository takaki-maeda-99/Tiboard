...
def get_thread(request, thread_id):
    if thread_id == 0:
        return JsonResponse({'thread': None})
            ...
            'posts': [
                {
                    ...
                    'reply_to': model_to_dict(post.reply_to, fields=['id', 'author']) if post.reply_to else None,
                    'author': model_to_dict(post.author, fields=['id', 'username']),
